      ...
      {({ isOpen, onClose }) => (
        <>
          <MenuButton
            as={"button"}
            disabled={isFetching}
            className={`w-fit bg-transparent !border order-2 md:order-3 !border-gray-200 rounded ${isOpen ? "!bg-gray-200" : "hover:!bg-gray-100"}`}
          >
          ...
          <MenuList maxWidth={350} className='!p-4 shadow-lg'>
            ...
            <p className='text-gray-500 !mt-1'>All posts are pulled from the following open source APIs.</p>
            <CheckboxGroup colorScheme='cyan' value={selectedSources} onChange={handleChange}>
              {checkboxList}
            </CheckboxGroup>
            <p className='!text-sm text-cyan-600'>Minimum of one selection required</p>
            <div className='!mt-3 grid grid-cols-2 gap-2'>
              ...
              <Button
                ...
                colorScheme='cyan'
                className='font-medium !rounded py-2 bg-cyan-600 !text-white'
